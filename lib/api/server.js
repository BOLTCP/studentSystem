...
app.post('/User/Login/Teacher', async (req, res) => {
  ...
  try {
    ...
    if (user) {;
      return res.status(200).json({ message: 'Амжилттай нэвтэрлээ', teacher, user, dep, dept_of_edu, teachers_major});
    } else {
      console.log('User not found');
      return res.status(401).json({ message: 'Алдаа гарлаа' });
    }
...
app.post('/Get/allMajor/Of/DepartmentsOfEducation', async (req, res) => {
  const { departments_of_edu_id, teacher_id } = req.body;
  console.log("Received Departments of Education Id of:", departments_of_edu_id, " and Teacher Id of:", teacher_id);
  ...
  try {
    ...
    const departmenst_of_dep_of_edu = await client.query(
      'SELECT * FROM department WHERE department_of_edu_id  = $1', [departments_of_edu_id]);
    ...
    departments = departmenst_of_dep_of_edu.rows;
    if (courses.length > 0) {
      return res.status(200).json({ message: 'Хөтөлбөрүүдийг амжилттай татлаа', courses, departments, teachers_major_selections});
    } else {
...
app.post('/Get/Current/Majors/Of/Teacher', async (req, res) => {
  ...
  console.log("Received Teacher Id of:", teacher_id, "Major Id of:", major_id);
...
app.post('/Remove/Major/From/Teacher', async (req, res) => {
  ...
  console.log("Received A Teacher Id of:", teacher_id + " to remove Major Id of:", majorId);
  ...
  try {
    ...
    const deleteResult = await client.query(majorToRemoveQuery, [teacher_id, majorId]);
...
app.post('/Get/AllCourses/Of/Teachers/Selected/Major', async (req, res) => {
  const { teacher_id, selected_major } = req.body;
  console.log('Received Teacher Id for CoursesPlanning:', teacher_id);
  console.log('Received selected_major:', selected_major);

  try {
    console.log("HERE");
    let teachersMajorPlanningList = [];
    if (selected_major) {
      try {
        teachersMajorPlanningList = JSON.parse(decodeURIComponent(selected_major));
        console.log('Decoded Teachers Major Planning:', teachersMajorPlanningList);
      } catch (err) {
        console.error('Error parsing selected_major:', err);
        return res.status(400).json({ message: 'Invalid selected_major format' });
      }
    }

    // Query the database for the teacher's current courses
    const current_courses_teacher = await client.query(
      'SELECT * FROM teacherscourseplanning WHERE teacher_id = $1',
      [teacher_id]
    );
    const current_courses = current_courses_teacher.rows;

    if (current_courses.length > 0) {
      console.log('Teacher has courses.');
      return res.status(200).json({ message: 'Teacher has courses', current_courses });
    } else {
      console.log('Teacher has no courses.');
      return res.status(404).json({ message: 'Teacher has no courses' });
    }
  } catch (error) {
    console.error('Database query failed:', error);
    return res.status(500).json({ message: 'Internal Server Error' });
...
app.get('/Get/Current/CoursesPlanning/Of/Teacher', async (req, res) => {
  const { teacher_id } = req.body;
  console.log('Recieved Teacher Id: for CoursesPlanning', teacher_id);
