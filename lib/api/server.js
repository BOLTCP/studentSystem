...
app.get('/Get/Teachers/Selected/Courses/Schedules', async (req, res) => {
  ...
  try {
    ...
    const teachers_made_schedules_query = await client.query(`
      SELECT time, days, classroom_id FROM teachersschedule WHERE classroom_id IS NOT NULL AND teacher_id = $1`, [teacher_id]);
    ...
    if (teachers_made_schedules_array.length > 0) {
      return res.status(200).json({ message: 'Багшийн хуваарийг амжилттай татлаа', teachers_made_schedules_array });
    } else {
