...
app.post('/User/Login/Teacher', async (req, res) => {
  ...
  try {
    ...
    console.log(user, teacher, dept_of_edu);
    if (user) {
      console.log('User found:', user);
      return res.status(200).json({ message: 'Амжилттай нэвтэрлээ', teacher, user, dep, dept_of_edu});
    } else {
      console.log('User not found');
      return res.status(401).json({ message: 'Алдаа гарлаа' });
    }
...
app.post('/Get/allMajor/Of/DepartmentsOfEducation', async (req, res) => {
  const { departments_of_edu_id } = req.body;
  console.log("Received Departments of Education Id of:", departments_of_edu_id);
  ...
  try {
    ...
    console.log(dep_of_edus_courses.rows, departmenst_of_dep_of_edu.rows);
    courses = dep_of_edus_courses.rows;
    departments = departmenst_of_dep_of_edu.rows;
    if (courses.length > 0) {
      console.log('Majors found:', courses);
      return res.status(200).json({ message: 'Хөтөлбөрүүдийг амжилттай татлаа', courses, departments});
    } else {
...
app.post('/Get/Current/Majors/Of/Teacher', async (req, res) => {
  ...
  try {
    ...
    console.log(current_majors_teacher.rows.length, add_to_teacher_major.rows);
    teachers_majors_count = current_majors_teacher.rows.length;
    current_majors = current_majors_teacher.rows;
    console.log(current_majors);
    console.log(teachers_majors_count);
    ...
    if (teachers_majors_count > 0) {
      ...
      console.log('Majors not found');
      return res.status(401).json({ message: 'Багшийн нэр дээр сонгосон хөтөлбөр байхгүй байна', a_major_to_be_added});
    }
  } catch (error) {
    console.error('Database query failed:', error);
    return res.status(500).json({ message: 'Internal Server Error' });
...
app.post('/Remove/Major/From/Teacher', async (req, res) => {
  ...
  console.log("Received A Teacher Id of:", teacher_id);

  try {
    const majorToRemoveQuery = `
      DELETE FROM teachersmajorplanning WHERE teacher_id = $1
      RETURNING teacher_id, major_name, major_id;
    `;

    const deleteResult = await client.query(majorToRemoveQuery, [teacher_id]);

    if (deleteResult.rows.length > 0) {
      const { teacher_id, major_name, major_id } = deleteResult.rows[0];

      return res.status(200).json({
        message: 'Мажор амжилттай хасагдлаа!',
        deleted_major: {
          teacher_id,        
          major_name,        
          major_id,          
...
// Get ALL courses data of teacher's selected major
app.post('Get/allCourses/Of/Major/Teacher', async (req, res) => {
  const { user_id } = req.body;
  console.log("Received user_id:", user_id);
  ...
  try {
    ...
    const department = await client.query('SELECT * FROM departmentsofeducation WHERE departments_of_education_id = $1', [teacher.departments_of_education_id]);
    const department_of_edu_query = department.rows[0];
    console.log(user, teacher, department_of_edu_query);
    if (user) {
      console.log('User found:', user);
      return res.status(200).json({ message: 'Амжилттай нэвтэрлээ', teacher, user, department_of_edu_query});
    } else {
      console.log('User not found');
      return res.status(401).json({ message: 'Алдаа гарлаа' });
    }
