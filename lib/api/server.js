      ...
      for (let i = 0; i < teachers_majors.length; i++) {
        ...
        try{
          const teachers_majors_courses_query = await client.query(`
            SELECT * FROM courses WHERE major_id = $1 ORDER BY course_year`, [teachers_majors[i].major_id] );
          teacherCourses.push(teachers_majors_courses_query.rows[0]);
