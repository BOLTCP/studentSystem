...
app.get('/Get/Teachers/Auto/Schedule', async (req, res) => {
  ...
  const { teacher_id } = req.query;
  console.log('Get/Teachers/Auto/Schedule Recieved Teacher Id: Auto Schedule of: ', teacher_id);
  ...
  try {
    ...
    for (i=0; i<coursesClassrooms.length; i+=2) {
      ...
      let checkForDuplicateClassrooms = await client.query(`
        ...
        course_id = $3`, [teacher.teacher_id, coursesClassrooms[jumper].classroom_id, coursesClassrooms[course_jumper].course_id,]);
        ...
        if (parseInt(checkForDuplicateClassrooms.rows[0].count) === 0) {
          ...
          teachers_email, time, course_planning_id, days, course_name, classroom_capacity, classroom_type,
          classroom_number
          ) VALUES (
          ...
          if (classroomToTeacherQuery.rows.length > 0) {
            ...
            console.log(`Багшийн ${coursesClassrooms[i+1].course_name} хичээлийг амжилтай 
              ${dayOfWeek} өдөрт ${periodOfDay} цагт ${coursesClassrooms[i].classroom_id} ангид нэмлээ.`);
          } else {
            errorMessages.push(`Багшийн ${coursesClassrooms[i+1].course_name} хичээлийг ${dayOfWeek} 
              өдөрт ${periodOfDay} цагт ${coursesClassrooms[i].classroom_id} ангид 
